/**
 * Creates prompt for error feedback and prepares openai post body
 * @param {*} input selection,action,expectedInput,studentInput
 * @returns post body for openai
 */
function prepareNLPBuggyMsg (input) {
    ...
    const prompt = 
        ...
        The student is attempting to convert the ${selection.includes("Num") ? "numerator" : "denominator"} of ${selection == "firstDenConv" || selection == "firstNumConv" ? "1/4" : "1/6"} to ${expectedInput}.
        However, they converted the ${selection.includes("Num") ? "numerator" : "denominator"} to ${studentInput} instead of ${expectedInput}, which is incorrect.
